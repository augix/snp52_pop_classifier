...
class theModel(nn.Module):
    ...
    def forward(self, contig1, contig2, emb1, emb2):
        ...
        contig2 = contig2 + torch.tensor(3000, dtype=contig2.dtype, device=contig2.device)
        ...
        x = torch.cat([x1, x2], dim=1) # [batch, seqlen*2, d_emb+d_id]
        x = self.correct_dim(x) # [batch, seqlen*2, d_model]
        for layer in self.layers:
            x = layer(x)
        # encoder output: [batch, seqlen*2, d_model]
        x = self.norm(x)
        x = x[:, 0, :]
