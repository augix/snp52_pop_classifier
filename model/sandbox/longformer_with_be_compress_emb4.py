...
class theModel(nn.Module):
    def __init__(self, config):
        ...
        self.compress1 = mlp(d_model, int(d_model/2+cd/2), cd)
        if self.config.add_cls:
            # self.compress2 = nn.Linear(seqlen+1, cs) # +1 for the cls token
            self.compress2 = mlp(seqlen+1, int(seqlen/2+cs/2), cs)
        else:
        ...
        head10_d1 = d_emb
        head11_d3 = seqlen*n_output_values
        head1_step = int(head11_d3/4 - head10_d1/4)
        head10_d2 = head10_d1 + head1_step
        head10_d3 = head10_d2 + head1_step
        head11_d1 = head10_d3
        head11_d2 = head11_d1 + head1_step
        self.head10 = mlp(head10_d1, head10_d2, head10_d3)
        self.head11 = mlp(head11_d1, head11_d2, head11_d3)
