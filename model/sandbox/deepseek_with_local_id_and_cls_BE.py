...
class theModel(nn.Module):
    def __init__(self, config):
        ...
        # create emb
        # decode from emb
        self.mlp = mlp(d_model, d_model, seqlen*n_output_values)
    ...
    def decode_from_emb(self, emb):
        # emb.shape: [batch, d_model]
        output = self.mlp(emb)
        # output.shape: [batch, seqlen*n_output_values]
        output = output.reshape(output.shape[0], -1, self.config.n_output_values)
        # output.shape: [batch, seqlen, n_output_values]
        return output
