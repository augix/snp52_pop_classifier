...
class theModel(nn.Module):
    ...
    def decode_from_emb(self, emb, id):
        ...
        # emb reshape to [batch, 1, d_model]
        emb = emb.reshape(emb.shape[0], 1, -1) # [batch, 1, d_model]
        # emb repeat to [batch, seqlen, d_model]
        emb = emb.repeat(1, id.shape[1], 1) # [batch, seqlen, d_model]
        # concat emb and id
        x = torch.cat([emb, id], dim=-1) # [batch, seqlen, d_model+d_id]
        x = self.mlp(x)
        x = self.norm(x)
        x = self.head(x) # [batch, seqlen, n_output_values]
        return x
    ...
    def forward(self, x, id):
        ...
        emb = self.create_emb(x) # [batch, d_model]
        logits = self.decode_from_emb(emb, id) # [batch, seqlen, n_output_values]
        result = {
            'logits0': logits0,
            'logits': logits,
        }
