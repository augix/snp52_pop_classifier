...
class theModel(nn.Module):
    def __init__(self, config):
        ...
        # vae
        self.vae_mu = mlp(cs*cd, cs*cd, cs*cd, config.dropout)
        self.vae_logvar = mlp(cs*cd, cs*cd, cs*cd, config.dropout)
    ...
    def reparameterize(self, mu, logvar):
        # mu.shape: [batch, cs*cd]
        # logvar.shape: [batch, cs*cd]
    ...
    def forward(self, contig, emb):
        ...
        # insert a vae for the compressed
        mu = self.vae_mu(compressed) # [batch, cs*cd]
        logvar = self.vae_logvar(compressed) # [batch, cs*cd]
        compressed = self.reparameterize(mu, logvar) # [batch, cs*cd]
