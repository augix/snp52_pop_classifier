...
class theModel(nn.Module):
    def __init__(self, config):
        ...
        self.compress1 = mlp(d_emb*2, cd//2+d_emb, cd, dropout=config.dropout)
        if config.add_cls:
            self.compress2 = mlp(seqlen+1, cs//2+seqlen//2, cs, dropout=config.dropout)
        else:
            self.compress2 = mlp(seqlen, cs//2+seqlen//2, cs, dropout=config.dropout)
        self.norm_emb = RMSNorm(cs*cd)
        self.head = mlp(cs*cd, n_output_values, n_output_values, dropout=config.dropout)
