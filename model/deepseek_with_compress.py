...
class theModel(nn.Module):
    def __init__(self, config):
        ...
        self.correct_dim = mlp(d_emb + d_id, d_model, d_model)
        ...
        self.compress1 = mlp(d_model, int(d_model/2+cd/2), cd, dropout=config.dropout)
        if self.config.add_cls:
            # self.compress2 = nn.Linear(seqlen+1, cs) # +1 for the cls token
            self.compress2 = mlp(seqlen+1, int(seqlen/2+cs/2), cs, dropout=config.dropout)
        else:
            # self.compress2 = nn.Linear(seqlen, cs)
            self.compress2 = mlp(seqlen, int(seqlen/2+cs/2), cs)
        self.norm_emb = RMSNorm(cd*cs)
        self.head = mlp(cd*cs, n_output_values, n_output_values, dropout=config.dropout)

    def create_emb(self, x):
        # x.shape: [batch, seqlen, d_model]
        x = self.compress1(x) # [batch, seqlen, cd]
        # permute to [batch, cd, seqlen]
        x = x.permute(0, 2, 1) # [batch, cd, seqlen]
        x = self.compress2(x) # [batch, cd, cs]
        # flatten
        x = x.view(x.size(0), -1) # [batch, cd*cs]
        return x
    ...
    def forward(self, contig1, contig2, emb1, emb2):
        ...
        # x = torch.cat([id1, emb1, id2, emb2], dim=-1) # [batch, seqlen, d_emb*2+d_id*2]

        id = self.id_embed(contig1) # [batch, seqlen, d_id]
        emb = emb1 - emb2
        x = torch.cat([id, emb], dim=-1) # [batch, seqlen, d_emb+d_id]
        x = self.correct_dim(x) # [batch, seqlen, d_model]
        for layer in self.layers:
            x = layer(x)
        # encoder output: [batch, seqlen, d_model]
        x = self.create_emb(x) # [batch, cd*cs]
