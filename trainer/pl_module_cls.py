...
class model_wrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        cls_logits = results['logits'][:, 0, :] # use cls token
        loss, acc = self._calculate_loss_acc(cls_logits, y, self.metric)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        cls_logits = results['logits'][:, 0, :] # use cls token
        loss, acc = self._calculate_loss_acc(cls_logits, y, self.metric)
        confusion_matrix = self.get_confusion_matrix(cls_logits, y)
        outputs = {
