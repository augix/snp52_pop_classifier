...
config = Namespace(
    ...
    bs_test = 128,                        # Reduced batch size for validation to save memory
    ...
    # model 
    model_py = 'model/substract_regression.py',  # Path to model module
    model_wrapper_py = 'trainer/pl_module_regression.py',  # Path to model wrapper module
...
config.ckpt_path = f'results/{config.name}/ckpt/epoch=01-val_acc=0.59150.ckpt'  # Path to specific checkpoint to load (set at runtime)
