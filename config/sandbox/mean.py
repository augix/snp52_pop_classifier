...
config = Namespace(
    ...
    cpu_per_worker = 4,                # Number of CPU cores per worker for data loading
    ...
    # model 
    model_py = 'model/deepseek_with_mean.py',  # Path to model module
    model_wrapper_py = 'trainer/pl_module.py',  # Path to model wrapper module
    n_layers = 4,                      # Number of transformer layers
    n_heads = 4,                       # Number of attention heads
    d_model = 64,                      # Model dimension
    d_ffn = 64*2,                      # Feed-forward network dimension
    ...
    max_grad_norm = 5,                 # Maximum gradient norm for clipping. 1.0 is conservative, 0.5 is aggressive, 0.0 is no clipping, 5.0 allows large gradients
