...
config = Namespace(
    ...
    populations = ['EUR'],
    add_cls = True,
    n_cls = 1,                         # Number of cls tokens reserved
    cpu_per_worker = 4,                # Number of CPU cores per worker for data loading
    bs_train = 24,                      # Reduced batch size for training to save memory
    bs_val = 32,                        # Reduced batch size for validation to save memory
    ...
    # model 
    model_py = 'model/deepseek_with_cls_stack.py',  # Path to model module
